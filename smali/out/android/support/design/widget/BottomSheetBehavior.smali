.class public Landroid/support/design/widget/BottomSheetBehavior;
.super Landroid/support/design/widget/CoordinatorLayout$a;


# annotations
.annotation system Ldalvik/annotation/MemberClasses;
    value = {
        Landroid/support/design/widget/BottomSheetBehavior$a;,
        Landroid/support/design/widget/BottomSheetBehavior$b;
    }
.end annotation

.annotation system Ldalvik/annotation/Signature;
    value = {
        "<V:",
        "Landroid/view/View;",
        ">",
        "Landroid/support/design/widget/CoordinatorLayout$a<",
        "TV;>;"
    }
.end annotation


# instance fields
.field a:I

.field b:I

.field c:Z

.field d:I

.field e:Landroid/support/v4/widget/l;

.field f:I

.field g:Ljava/lang/ref/WeakReference;
    .annotation system Ldalvik/annotation/Signature;
        value = {
            "Ljava/lang/ref/WeakReference<",
            "TV;>;"
        }
    .end annotation
.end field

.field h:Ljava/lang/ref/WeakReference;
    .annotation system Ldalvik/annotation/Signature;
        value = {
            "Ljava/lang/ref/WeakReference<",
            "Landroid/view/View;",
            ">;"
        }
    .end annotation
.end field

.field i:I

.field j:Z

.field private k:F

.field private l:I

.field private m:Z

.field private n:I

.field private o:Z

.field private p:Z

.field private q:I

.field private r:Z

.field private s:Landroid/view/VelocityTracker;

.field private t:I

.field private final u:Landroid/support/v4/widget/l$a;


# direct methods
.method public constructor <init>()V
    .registers 2

    invoke-direct {p0}, Landroid/support/design/widget/CoordinatorLayout$a;-><init>()V

    const/4 v0, 0x4

    iput v0, p0, Landroid/support/design/widget/BottomSheetBehavior;->d:I

    new-instance v0, Landroid/support/design/widget/BottomSheetBehavior$1;

    invoke-direct {v0, p0}, Landroid/support/design/widget/BottomSheetBehavior$1;-><init>(Landroid/support/design/widget/BottomSheetBehavior;)V

    iput-object v0, p0, Landroid/support/design/widget/BottomSheetBehavior;->u:Landroid/support/v4/widget/l$a;

    return-void
.end method

.method public constructor <init>(Landroid/content/Context;Landroid/util/AttributeSet;)V
    .registers 6

    invoke-direct {p0, p1, p2}, Landroid/support/design/widget/CoordinatorLayout$a;-><init>(Landroid/content/Context;Landroid/util/AttributeSet;)V

    const/4 v0, 0x4

    iput v0, p0, Landroid/support/design/widget/BottomSheetBehavior;->d:I

    new-instance v0, Landroid/support/design/widget/BottomSheetBehavior$1;

    invoke-direct {v0, p0}, Landroid/support/design/widget/BottomSheetBehavior$1;-><init>(Landroid/support/design/widget/BottomSheetBehavior;)V

    iput-object v0, p0, Landroid/support/design/widget/BottomSheetBehavior;->u:Landroid/support/v4/widget/l$a;

    sget-object v0, Landroid/support/design/a$i;->BottomSheetBehavior_Layout:[I

    invoke-virtual {p1, p2, v0}, Landroid/content/Context;->obtainStyledAttributes(Landroid/util/AttributeSet;[I)Landroid/content/res/TypedArray;

    move-result-object p2

    sget v0, Landroid/support/design/a$i;->BottomSheetBehavior_Layout_behavior_peekHeight:I

    invoke-virtual {p2, v0}, Landroid/content/res/TypedArray;->peekValue(I)Landroid/util/TypedValue;

    move-result-object v0

    const/4 v1, -0x1

    if-eqz v0, :cond_23

    iget v2, v0, Landroid/util/TypedValue;->data:I

    if-ne v2, v1, :cond_23

    iget v0, v0, Landroid/util/TypedValue;->data:I

    goto :goto_29

    :cond_23
    sget v0, Landroid/support/design/a$i;->BottomSheetBehavior_Layout_behavior_peekHeight:I

    invoke-virtual {p2, v0, v1}, Landroid/content/res/TypedArray;->getDimensionPixelSize(II)I

    move-result v0

    :goto_29
    invoke-direct {p0, v0}, Landroid/support/design/widget/BottomSheetBehavior;->c(I)V

    sget v0, Landroid/support/design/a$i;->BottomSheetBehavior_Layout_behavior_hideable:I

    const/4 v1, 0x0

    invoke-virtual {p2, v0, v1}, Landroid/content/res/TypedArray;->getBoolean(IZ)Z

    move-result v0

    iput-boolean v0, p0, Landroid/support/design/widget/BottomSheetBehavior;->c:Z

    sget v0, Landroid/support/design/a$i;->BottomSheetBehavior_Layout_behavior_skipCollapsed:I

    invoke-virtual {p2, v0, v1}, Landroid/content/res/TypedArray;->getBoolean(IZ)Z

    move-result v0

    iput-boolean v0, p0, Landroid/support/design/widget/BottomSheetBehavior;->o:Z

    invoke-virtual {p2}, Landroid/content/res/TypedArray;->recycle()V

    invoke-static {p1}, Landroid/view/ViewConfiguration;->get(Landroid/content/Context;)Landroid/view/ViewConfiguration;

    move-result-object p1

    invoke-virtual {p1}, Landroid/view/ViewConfiguration;->getScaledMaximumFlingVelocity()I

    move-result p1

    int-to-float p1, p1

    iput p1, p0, Landroid/support/design/widget/BottomSheetBehavior;->k:F

    return-void
.end method

.method private b(Landroid/view/View;)Landroid/view/View;
    .registers 5

    invoke-static {p1}, Landroid/support/v4/i/p;->q(Landroid/view/View;)Z

    move-result v0

    if-eqz v0, :cond_7

    return-object p1

    :cond_7
    instance-of v0, p1, Landroid/view/ViewGroup;

    if-eqz v0, :cond_22

    check-cast p1, Landroid/view/ViewGroup;

    const/4 v0, 0x0

    invoke-virtual {p1}, Landroid/view/ViewGroup;->getChildCount()I

    move-result v1

    :goto_12
    if-ge v0, v1, :cond_22

    invoke-virtual {p1, v0}, Landroid/view/ViewGroup;->getChildAt(I)Landroid/view/View;

    move-result-object v2

    invoke-direct {p0, v2}, Landroid/support/design/widget/BottomSheetBehavior;->b(Landroid/view/View;)Landroid/view/View;

    move-result-object v2

    if-eqz v2, :cond_1f

    return-object v2

    :cond_1f
    add-int/lit8 v0, v0, 0x1

    goto :goto_12

    :cond_22
    const/4 p1, 0x0

    return-object p1
.end method

.method private b()V
    .registers 2

    const/4 v0, -0x1

    iput v0, p0, Landroid/support/design/widget/BottomSheetBehavior;->i:I

    iget-object v0, p0, Landroid/support/design/widget/BottomSheetBehavior;->s:Landroid/view/VelocityTracker;

    if-eqz v0, :cond_d

    invoke-virtual {v0}, Landroid/view/VelocityTracker;->recycle()V

    const/4 v0, 0x0

    iput-object v0, p0, Landroid/support/design/widget/BottomSheetBehavior;->s:Landroid/view/VelocityTracker;

    :cond_d
    return-void
.end method

.method private c(I)V
    .registers 5

    const/4 v0, 0x1

    const/4 v1, 0x0

    const/4 v2, -0x1

    if-ne p1, v2, :cond_c

    iget-boolean p1, p0, Landroid/support/design/widget/BottomSheetBehavior;->m:Z

    if-nez p1, :cond_15

    iput-boolean v0, p0, Landroid/support/design/widget/BottomSheetBehavior;->m:Z

    goto :goto_24

    :cond_c
    iget-boolean v2, p0, Landroid/support/design/widget/BottomSheetBehavior;->m:Z

    if-nez v2, :cond_17

    iget v2, p0, Landroid/support/design/widget/BottomSheetBehavior;->l:I

    if-eq v2, p1, :cond_15

    goto :goto_17

    :cond_15
    const/4 v0, 0x0

    goto :goto_24

    :cond_17
    :goto_17
    iput-boolean v1, p0, Landroid/support/design/widget/BottomSheetBehavior;->m:Z

    invoke-static {v1, p1}, Ljava/lang/Math;->max(II)I

    move-result v1

    iput v1, p0, Landroid/support/design/widget/BottomSheetBehavior;->l:I

    iget v1, p0, Landroid/support/design/widget/BottomSheetBehavior;->f:I

    sub-int/2addr v1, p1

    iput v1, p0, Landroid/support/design/widget/BottomSheetBehavior;->b:I

    :goto_24
    if-eqz v0, :cond_3a

    iget p1, p0, Landroid/support/design/widget/BottomSheetBehavior;->d:I

    const/4 v0, 0x4

    if-ne p1, v0, :cond_3a

    iget-object p1, p0, Landroid/support/design/widget/BottomSheetBehavior;->g:Ljava/lang/ref/WeakReference;

    if-eqz p1, :cond_3a

    invoke-virtual {p1}, Ljava/lang/ref/WeakReference;->get()Ljava/lang/Object;

    move-result-object p1

    check-cast p1, Landroid/view/View;

    if-eqz p1, :cond_3a

    invoke-virtual {p1}, Landroid/view/View;->requestLayout()V

    :cond_3a
    return-void
.end method


# virtual methods
.method final a()V
    .registers 2

    iget-object v0, p0, Landroid/support/design/widget/BottomSheetBehavior;->g:Ljava/lang/ref/WeakReference;

    invoke-virtual {v0}, Ljava/lang/ref/WeakReference;->get()Ljava/lang/Object;

    return-void
.end method

.method public final a(Landroid/support/design/widget/CoordinatorLayout;Landroid/view/View;Landroid/os/Parcelable;)V
    .registers 5
    .annotation system Ldalvik/annotation/Signature;
        value = {
            "(",
            "Landroid/support/design/widget/CoordinatorLayout;",
            "TV;",
            "Landroid/os/Parcelable;",
            ")V"
        }
    .end annotation

    check-cast p3, Landroid/support/design/widget/BottomSheetBehavior$a;

    iget-object v0, p3, Landroid/support/v4/i/a;->e:Landroid/os/Parcelable;

    invoke-super {p0, p1, p2, v0}, Landroid/support/design/widget/CoordinatorLayout$a;->a(Landroid/support/design/widget/CoordinatorLayout;Landroid/view/View;Landroid/os/Parcelable;)V

    iget p1, p3, Landroid/support/design/widget/BottomSheetBehavior$a;->a:I

    const/4 p2, 0x1

    if-eq p1, p2, :cond_17

    iget p1, p3, Landroid/support/design/widget/BottomSheetBehavior$a;->a:I

    const/4 p2, 0x2

    if-ne p1, p2, :cond_12

    goto :goto_17

    :cond_12
    iget p1, p3, Landroid/support/design/widget/BottomSheetBehavior$a;->a:I

    :goto_14
    iput p1, p0, Landroid/support/design/widget/BottomSheetBehavior;->d:I

    return-void

    :cond_17
    :goto_17
    const/4 p1, 0x4

    goto :goto_14
.end method

.method public final a(Landroid/view/View;Landroid/view/View;)V
    .registers 7
    .annotation system Ldalvik/annotation/Signature;
        value = {
            "(TV;",
            "Landroid/view/View;",
            ")V"
        }
    .end annotation

    invoke-virtual {p1}, Landroid/view/View;->getTop()I

    move-result v0

    iget v1, p0, Landroid/support/design/widget/BottomSheetBehavior;->a:I

    const/4 v2, 0x3

    if-ne v0, v1, :cond_d

    invoke-virtual {p0, v2}, Landroid/support/design/widget/BottomSheetBehavior;->b(I)V

    return-void

    :cond_d
    iget-object v0, p0, Landroid/support/design/widget/BottomSheetBehavior;->h:Ljava/lang/ref/WeakReference;

    if-eqz v0, :cond_92

    invoke-virtual {v0}, Ljava/lang/ref/WeakReference;->get()Ljava/lang/Object;

    move-result-object v0

    if-ne p2, v0, :cond_92

    iget-boolean p2, p0, Landroid/support/design/widget/BottomSheetBehavior;->r:Z

    if-nez p2, :cond_1d

    goto/16 :goto_92

    :cond_1d
    iget p2, p0, Landroid/support/design/widget/BottomSheetBehavior;->q:I

    const/4 v0, 0x4

    if-lez p2, :cond_25

    :goto_22
    iget p2, p0, Landroid/support/design/widget/BottomSheetBehavior;->a:I

    goto :goto_61

    :cond_25
    iget-boolean p2, p0, Landroid/support/design/widget/BottomSheetBehavior;->c:Z

    if-eqz p2, :cond_44

    iget-object p2, p0, Landroid/support/design/widget/BottomSheetBehavior;->s:Landroid/view/VelocityTracker;

    const/16 v1, 0x3e8

    iget v3, p0, Landroid/support/design/widget/BottomSheetBehavior;->k:F

    invoke-virtual {p2, v1, v3}, Landroid/view/VelocityTracker;->computeCurrentVelocity(IF)V

    iget-object p2, p0, Landroid/support/design/widget/BottomSheetBehavior;->s:Landroid/view/VelocityTracker;

    iget v1, p0, Landroid/support/design/widget/BottomSheetBehavior;->i:I

    invoke-virtual {p2, v1}, Landroid/view/VelocityTracker;->getYVelocity(I)F

    move-result p2

    invoke-virtual {p0, p1, p2}, Landroid/support/design/widget/BottomSheetBehavior;->a(Landroid/view/View;F)Z

    move-result p2

    if-eqz p2, :cond_44

    iget p2, p0, Landroid/support/design/widget/BottomSheetBehavior;->f:I

    const/4 v2, 0x5

    goto :goto_61

    :cond_44
    iget p2, p0, Landroid/support/design/widget/BottomSheetBehavior;->q:I

    if-nez p2, :cond_5e

    invoke-virtual {p1}, Landroid/view/View;->getTop()I

    move-result p2

    iget v1, p0, Landroid/support/design/widget/BottomSheetBehavior;->a:I

    sub-int v1, p2, v1

    invoke-static {v1}, Ljava/lang/Math;->abs(I)I

    move-result v1

    iget v3, p0, Landroid/support/design/widget/BottomSheetBehavior;->b:I

    sub-int/2addr p2, v3

    invoke-static {p2}, Ljava/lang/Math;->abs(I)I

    move-result p2

    if-ge v1, p2, :cond_5e

    goto :goto_22

    :cond_5e
    iget p2, p0, Landroid/support/design/widget/BottomSheetBehavior;->b:I

    const/4 v2, 0x4

    :goto_61
    iget-object v0, p0, Landroid/support/design/widget/BottomSheetBehavior;->e:Landroid/support/v4/widget/l;

    invoke-virtual {p1}, Landroid/view/View;->getLeft()I

    move-result v1

    iput-object p1, v0, Landroid/support/v4/widget/l;->d:Landroid/view/View;

    const/4 v3, -0x1

    iput v3, v0, Landroid/support/v4/widget/l;->c:I

    const/4 v3, 0x0

    invoke-virtual {v0, v1, p2, v3, v3}, Landroid/support/v4/widget/l;->a(IIII)Z

    move-result p2

    if-nez p2, :cond_7e

    iget v1, v0, Landroid/support/v4/widget/l;->a:I

    if-nez v1, :cond_7e

    iget-object v1, v0, Landroid/support/v4/widget/l;->d:Landroid/view/View;

    if-eqz v1, :cond_7e

    const/4 v1, 0x0

    iput-object v1, v0, Landroid/support/v4/widget/l;->d:Landroid/view/View;

    :cond_7e
    if-eqz p2, :cond_8d

    const/4 p2, 0x2

    invoke-virtual {p0, p2}, Landroid/support/design/widget/BottomSheetBehavior;->b(I)V

    new-instance p2, Landroid/support/design/widget/BottomSheetBehavior$b;

    invoke-direct {p2, p0, p1, v2}, Landroid/support/design/widget/BottomSheetBehavior$b;-><init>(Landroid/support/design/widget/BottomSheetBehavior;Landroid/view/View;I)V

    invoke-static {p1, p2}, Landroid/support/v4/i/p;->a(Landroid/view/View;Ljava/lang/Runnable;)V

    goto :goto_90

    :cond_8d
    invoke-virtual {p0, v2}, Landroid/support/design/widget/BottomSheetBehavior;->b(I)V

    :goto_90
    iput-boolean v3, p0, Landroid/support/design/widget/BottomSheetBehavior;->r:Z

    :cond_92
    :goto_92
    return-void
.end method

.method public final a(Landroid/view/View;Landroid/view/View;I[I)V
    .registers 9
    .annotation system Ldalvik/annotation/Signature;
        value = {
            "(TV;",
            "Landroid/view/View;",
            "I[I)V"
        }
    .end annotation

    iget-object v0, p0, Landroid/support/design/widget/BottomSheetBehavior;->h:Ljava/lang/ref/WeakReference;

    invoke-virtual {v0}, Ljava/lang/ref/WeakReference;->get()Ljava/lang/Object;

    move-result-object v0

    check-cast v0, Landroid/view/View;

    if-eq p2, v0, :cond_b

    return-void

    :cond_b
    invoke-virtual {p1}, Landroid/view/View;->getTop()I

    move-result v0

    sub-int v1, v0, p3

    const/4 v2, 0x1

    if-lez p3, :cond_30

    iget p2, p0, Landroid/support/design/widget/BottomSheetBehavior;->a:I

    if-ge v1, p2, :cond_26

    sub-int/2addr v0, p2

    aput v0, p4, v2

    aget p2, p4, v2

    neg-int p2, p2

    invoke-static {p1, p2}, Landroid/support/v4/i/p;->b(Landroid/view/View;I)V

    const/4 p2, 0x3

    :goto_22
    invoke-virtual {p0, p2}, Landroid/support/design/widget/BottomSheetBehavior;->b(I)V

    goto :goto_50

    :cond_26
    aput p3, p4, v2

    :goto_28
    neg-int p2, p3

    invoke-static {p1, p2}, Landroid/support/v4/i/p;->b(Landroid/view/View;I)V

    invoke-virtual {p0, v2}, Landroid/support/design/widget/BottomSheetBehavior;->b(I)V

    goto :goto_50

    :cond_30
    if-gez p3, :cond_50

    const/4 v3, -0x1

    invoke-virtual {p2, v3}, Landroid/view/View;->canScrollVertically(I)Z

    move-result p2

    if-nez p2, :cond_50

    iget p2, p0, Landroid/support/design/widget/BottomSheetBehavior;->b:I

    if-le v1, p2, :cond_4d

    iget-boolean v1, p0, Landroid/support/design/widget/BottomSheetBehavior;->c:Z

    if-eqz v1, :cond_42

    goto :goto_4d

    :cond_42
    sub-int/2addr v0, p2

    aput v0, p4, v2

    aget p2, p4, v2

    neg-int p2, p2

    invoke-static {p1, p2}, Landroid/support/v4/i/p;->b(Landroid/view/View;I)V

    const/4 p2, 0x4

    goto :goto_22

    :cond_4d
    :goto_4d
    aput p3, p4, v2

    goto :goto_28

    :cond_50
    :goto_50
    invoke-virtual {p1}, Landroid/view/View;->getTop()I

    invoke-virtual {p0}, Landroid/support/design/widget/BottomSheetBehavior;->a()V

    iput p3, p0, Landroid/support/design/widget/BottomSheetBehavior;->q:I

    iput-boolean v2, p0, Landroid/support/design/widget/BottomSheetBehavior;->r:Z

    return-void
.end method

.method public final a(I)Z
    .registers 3
    .annotation system Ldalvik/annotation/Signature;
        value = {
            "(I)Z"
        }
    .end annotation

    const/4 v0, 0x0

    iput v0, p0, Landroid/support/design/widget/BottomSheetBehavior;->q:I

    iput-boolean v0, p0, Landroid/support/design/widget/BottomSheetBehavior;->r:Z

    and-int/lit8 p1, p1, 0x2

    if-eqz p1, :cond_b

    const/4 p1, 0x1

    return p1

    :cond_b
    return v0
.end method

.method public final a(Landroid/support/design/widget/CoordinatorLayout;Landroid/view/View;I)Z
    .registers 8
    .annotation system Ldalvik/annotation/Signature;
        value = {
            "(",
            "Landroid/support/design/widget/CoordinatorLayout;",
            "TV;I)Z"
        }
    .end annotation

    invoke-static {p1}, Landroid/support/v4/i/p;->l(Landroid/view/View;)Z

    move-result v0

    if-eqz v0, :cond_f

    invoke-static {p2}, Landroid/support/v4/i/p;->l(Landroid/view/View;)Z

    move-result v0

    if-nez v0, :cond_f

    invoke-static {p2}, Landroid/support/v4/i/p;->m(Landroid/view/View;)V

    :cond_f
    invoke-virtual {p2}, Landroid/view/View;->getTop()I

    move-result v0

    invoke-virtual {p1, p2, p3}, Landroid/support/design/widget/CoordinatorLayout;->a(Landroid/view/View;I)V

    invoke-virtual {p1}, Landroid/support/design/widget/CoordinatorLayout;->getHeight()I

    move-result p3

    iput p3, p0, Landroid/support/design/widget/BottomSheetBehavior;->f:I

    iget-boolean p3, p0, Landroid/support/design/widget/BottomSheetBehavior;->m:Z

    if-eqz p3, :cond_42

    iget p3, p0, Landroid/support/design/widget/BottomSheetBehavior;->n:I

    if-nez p3, :cond_30

    invoke-virtual {p1}, Landroid/support/design/widget/CoordinatorLayout;->getResources()Landroid/content/res/Resources;

    move-result-object p3

    sget v1, Landroid/support/design/a$c;->design_bottom_sheet_peek_height_min:I

    invoke-virtual {p3, v1}, Landroid/content/res/Resources;->getDimensionPixelSize(I)I

    move-result p3

    iput p3, p0, Landroid/support/design/widget/BottomSheetBehavior;->n:I

    :cond_30
    iget p3, p0, Landroid/support/design/widget/BottomSheetBehavior;->n:I

    iget v1, p0, Landroid/support/design/widget/BottomSheetBehavior;->f:I

    invoke-virtual {p1}, Landroid/support/design/widget/CoordinatorLayout;->getWidth()I

    move-result v2

    mul-int/lit8 v2, v2, 0x9

    div-int/lit8 v2, v2, 0x10

    sub-int/2addr v1, v2

    invoke-static {p3, v1}, Ljava/lang/Math;->max(II)I

    move-result p3

    goto :goto_44

    :cond_42
    iget p3, p0, Landroid/support/design/widget/BottomSheetBehavior;->l:I

    :goto_44
    const/4 v1, 0x0

    iget v2, p0, Landroid/support/design/widget/BottomSheetBehavior;->f:I

    invoke-virtual {p2}, Landroid/view/View;->getHeight()I

    move-result v3

    sub-int/2addr v2, v3

    invoke-static {v1, v2}, Ljava/lang/Math;->max(II)I

    move-result v1

    iput v1, p0, Landroid/support/design/widget/BottomSheetBehavior;->a:I

    iget v1, p0, Landroid/support/design/widget/BottomSheetBehavior;->f:I

    sub-int/2addr v1, p3

    iget p3, p0, Landroid/support/design/widget/BottomSheetBehavior;->a:I

    invoke-static {v1, p3}, Ljava/lang/Math;->max(II)I

    move-result p3

    iput p3, p0, Landroid/support/design/widget/BottomSheetBehavior;->b:I

    iget p3, p0, Landroid/support/design/widget/BottomSheetBehavior;->d:I

    const/4 v1, 0x3

    const/4 v2, 0x1

    if-ne p3, v1, :cond_69

    iget p3, p0, Landroid/support/design/widget/BottomSheetBehavior;->a:I

    :goto_65
    invoke-static {p2, p3}, Landroid/support/v4/i/p;->b(Landroid/view/View;I)V

    goto :goto_88

    :cond_69
    iget-boolean v1, p0, Landroid/support/design/widget/BottomSheetBehavior;->c:Z

    if-eqz v1, :cond_73

    const/4 v1, 0x5

    if-ne p3, v1, :cond_73

    iget p3, p0, Landroid/support/design/widget/BottomSheetBehavior;->f:I

    goto :goto_65

    :cond_73
    iget p3, p0, Landroid/support/design/widget/BottomSheetBehavior;->d:I

    const/4 v1, 0x4

    if-ne p3, v1, :cond_7b

    iget p3, p0, Landroid/support/design/widget/BottomSheetBehavior;->b:I

    goto :goto_65

    :cond_7b
    if-eq p3, v2, :cond_80

    const/4 v1, 0x2

    if-ne p3, v1, :cond_88

    :cond_80
    invoke-virtual {p2}, Landroid/view/View;->getTop()I

    move-result p3

    sub-int/2addr v0, p3

    invoke-static {p2, v0}, Landroid/support/v4/i/p;->b(Landroid/view/View;I)V

    :cond_88
    :goto_88
    iget-object p3, p0, Landroid/support/design/widget/BottomSheetBehavior;->e:Landroid/support/v4/widget/l;

    if-nez p3, :cond_94

    iget-object p3, p0, Landroid/support/design/widget/BottomSheetBehavior;->u:Landroid/support/v4/widget/l$a;

    invoke-static {p1, p3}, Landroid/support/v4/widget/l;->a(Landroid/view/ViewGroup;Landroid/support/v4/widget/l$a;)Landroid/support/v4/widget/l;

    move-result-object p1

    iput-object p1, p0, Landroid/support/design/widget/BottomSheetBehavior;->e:Landroid/support/v4/widget/l;

    :cond_94
    new-instance p1, Ljava/lang/ref/WeakReference;

    invoke-direct {p1, p2}, Ljava/lang/ref/WeakReference;-><init>(Ljava/lang/Object;)V

    iput-object p1, p0, Landroid/support/design/widget/BottomSheetBehavior;->g:Ljava/lang/ref/WeakReference;

    new-instance p1, Ljava/lang/ref/WeakReference;

    invoke-direct {p0, p2}, Landroid/support/design/widget/BottomSheetBehavior;->b(Landroid/view/View;)Landroid/view/View;

    move-result-object p2

    invoke-direct {p1, p2}, Ljava/lang/ref/WeakReference;-><init>(Ljava/lang/Object;)V

    iput-object p1, p0, Landroid/support/design/widget/BottomSheetBehavior;->h:Ljava/lang/ref/WeakReference;

    return v2
.end method

.method public final a(Landroid/support/design/widget/CoordinatorLayout;Landroid/view/View;Landroid/view/MotionEvent;)Z
    .registers 11
    .annotation system Ldalvik/annotation/Signature;
        value = {
            "(",
            "Landroid/support/design/widget/CoordinatorLayout;",
            "TV;",
            "Landroid/view/MotionEvent;",
            ")Z"
        }
    .end annotation

    invoke-virtual {p2}, Landroid/view/View;->isShown()Z

    move-result v0

    const/4 v1, 0x0

    const/4 v2, 0x1

    if-nez v0, :cond_b

    iput-boolean v2, p0, Landroid/support/design/widget/BottomSheetBehavior;->p:Z

    return v1

    :cond_b
    invoke-virtual {p3}, Landroid/view/MotionEvent;->getActionMasked()I

    move-result v0

    if-nez v0, :cond_14

    invoke-direct {p0}, Landroid/support/design/widget/BottomSheetBehavior;->b()V

    :cond_14
    iget-object v3, p0, Landroid/support/design/widget/BottomSheetBehavior;->s:Landroid/view/VelocityTracker;

    if-nez v3, :cond_1e

    invoke-static {}, Landroid/view/VelocityTracker;->obtain()Landroid/view/VelocityTracker;

    move-result-object v3

    iput-object v3, p0, Landroid/support/design/widget/BottomSheetBehavior;->s:Landroid/view/VelocityTracker;

    :cond_1e
    iget-object v3, p0, Landroid/support/design/widget/BottomSheetBehavior;->s:Landroid/view/VelocityTracker;

    invoke-virtual {v3, p3}, Landroid/view/VelocityTracker;->addMovement(Landroid/view/MotionEvent;)V

    const/4 v3, 0x3

    const/4 v4, -0x1

    if-eq v0, v3, :cond_6b

    packed-switch v0, :pswitch_data_c0

    goto :goto_76

    :pswitch_2b
    invoke-virtual {p3}, Landroid/view/MotionEvent;->getX()F

    move-result v3

    float-to-int v3, v3

    invoke-virtual {p3}, Landroid/view/MotionEvent;->getY()F

    move-result v5

    float-to-int v5, v5

    iput v5, p0, Landroid/support/design/widget/BottomSheetBehavior;->t:I

    iget-object v5, p0, Landroid/support/design/widget/BottomSheetBehavior;->h:Ljava/lang/ref/WeakReference;

    if-eqz v5, :cond_42

    invoke-virtual {v5}, Ljava/lang/ref/WeakReference;->get()Ljava/lang/Object;

    move-result-object v5

    check-cast v5, Landroid/view/View;

    goto :goto_43

    :cond_42
    const/4 v5, 0x0

    :goto_43
    if-eqz v5, :cond_59

    iget v6, p0, Landroid/support/design/widget/BottomSheetBehavior;->t:I

    invoke-virtual {p1, v5, v3, v6}, Landroid/support/design/widget/CoordinatorLayout;->a(Landroid/view/View;II)Z

    move-result v5

    if-eqz v5, :cond_59

    invoke-virtual {p3}, Landroid/view/MotionEvent;->getActionIndex()I

    move-result v5

    invoke-virtual {p3, v5}, Landroid/view/MotionEvent;->getPointerId(I)I

    move-result v5

    iput v5, p0, Landroid/support/design/widget/BottomSheetBehavior;->i:I

    iput-boolean v2, p0, Landroid/support/design/widget/BottomSheetBehavior;->j:Z

    :cond_59
    iget v5, p0, Landroid/support/design/widget/BottomSheetBehavior;->i:I

    if-ne v5, v4, :cond_67

    iget v4, p0, Landroid/support/design/widget/BottomSheetBehavior;->t:I

    invoke-virtual {p1, p2, v3, v4}, Landroid/support/design/widget/CoordinatorLayout;->a(Landroid/view/View;II)Z

    move-result p2

    if-nez p2, :cond_67

    const/4 p2, 0x1

    goto :goto_68

    :cond_67
    const/4 p2, 0x0

    :goto_68
    iput-boolean p2, p0, Landroid/support/design/widget/BottomSheetBehavior;->p:Z

    goto :goto_76

    :cond_6b
    :pswitch_6b
    iput-boolean v1, p0, Landroid/support/design/widget/BottomSheetBehavior;->j:Z

    iput v4, p0, Landroid/support/design/widget/BottomSheetBehavior;->i:I

    iget-boolean p2, p0, Landroid/support/design/widget/BottomSheetBehavior;->p:Z

    if-eqz p2, :cond_76

    iput-boolean v1, p0, Landroid/support/design/widget/BottomSheetBehavior;->p:Z

    return v1

    :cond_76
    :goto_76
    iget-boolean p2, p0, Landroid/support/design/widget/BottomSheetBehavior;->p:Z

    if-nez p2, :cond_83

    iget-object p2, p0, Landroid/support/design/widget/BottomSheetBehavior;->e:Landroid/support/v4/widget/l;

    invoke-virtual {p2, p3}, Landroid/support/v4/widget/l;->a(Landroid/view/MotionEvent;)Z

    move-result p2

    if-eqz p2, :cond_83

    return v2

    :cond_83
    iget-object p2, p0, Landroid/support/design/widget/BottomSheetBehavior;->h:Ljava/lang/ref/WeakReference;

    invoke-virtual {p2}, Ljava/lang/ref/WeakReference;->get()Ljava/lang/Object;

    move-result-object p2

    check-cast p2, Landroid/view/View;

    const/4 v3, 0x2

    if-ne v0, v3, :cond_be

    if-eqz p2, :cond_be

    iget-boolean v0, p0, Landroid/support/design/widget/BottomSheetBehavior;->p:Z

    if-nez v0, :cond_be

    iget v0, p0, Landroid/support/design/widget/BottomSheetBehavior;->d:I

    if-eq v0, v2, :cond_be

    invoke-virtual {p3}, Landroid/view/MotionEvent;->getX()F

    move-result v0

    float-to-int v0, v0

    invoke-virtual {p3}, Landroid/view/MotionEvent;->getY()F

    move-result v3

    float-to-int v3, v3

    invoke-virtual {p1, p2, v0, v3}, Landroid/support/design/widget/CoordinatorLayout;->a(Landroid/view/View;II)Z

    move-result p1

    if-nez p1, :cond_be

    iget p1, p0, Landroid/support/design/widget/BottomSheetBehavior;->t:I

    int-to-float p1, p1

    invoke-virtual {p3}, Landroid/view/MotionEvent;->getY()F

    move-result p2

    sub-float/2addr p1, p2

    invoke-static {p1}, Ljava/lang/Math;->abs(F)F

    move-result p1

    iget-object p2, p0, Landroid/support/design/widget/BottomSheetBehavior;->e:Landroid/support/v4/widget/l;

    iget p2, p2, Landroid/support/v4/widget/l;->b:I

    int-to-float p2, p2

    cmpl-float p1, p1, p2

    if-lez p1, :cond_be

    return v2

    :cond_be
    return v1

    nop

    :pswitch_data_c0
    .packed-switch 0x0
        :pswitch_2b
        :pswitch_6b
    .end packed-switch
.end method

.method public final a(Landroid/support/design/widget/CoordinatorLayout;Landroid/view/View;Landroid/view/View;FF)Z
    .registers 8
    .annotation system Ldalvik/annotation/Signature;
        value = {
            "(",
            "Landroid/support/design/widget/CoordinatorLayout;",
            "TV;",
            "Landroid/view/View;",
            "FF)Z"
        }
    .end annotation

    iget-object v0, p0, Landroid/support/design/widget/BottomSheetBehavior;->h:Ljava/lang/ref/WeakReference;

    invoke-virtual {v0}, Ljava/lang/ref/WeakReference;->get()Ljava/lang/Object;

    move-result-object v0

    if-ne p3, v0, :cond_15

    iget v0, p0, Landroid/support/design/widget/BottomSheetBehavior;->d:I

    const/4 v1, 0x3

    if-ne v0, v1, :cond_13

    invoke-super/range {p0 .. p5}, Landroid/support/design/widget/CoordinatorLayout$a;->a(Landroid/support/design/widget/CoordinatorLayout;Landroid/view/View;Landroid/view/View;FF)Z

    move-result p1

    if-eqz p1, :cond_15

    :cond_13
    const/4 p1, 0x1

    return p1

    :cond_15
    const/4 p1, 0x0

    return p1
.end method

.method final a(Landroid/view/View;F)Z
    .registers 7

    iget-boolean v0, p0, Landroid/support/design/widget/BottomSheetBehavior;->o:Z

    const/4 v1, 0x1

    if-eqz v0, :cond_6

    return v1

    :cond_6
    invoke-virtual {p1}, Landroid/view/View;->getTop()I

    move-result v0

    iget v2, p0, Landroid/support/design/widget/BottomSheetBehavior;->b:I

    const/4 v3, 0x0

    if-ge v0, v2, :cond_10

    return v3

    :cond_10
    invoke-virtual {p1}, Landroid/view/View;->getTop()I

    move-result p1

    int-to-float p1, p1

    const v0, 0x3dcccccd    # 0.1f

    mul-float p2, p2, v0

    add-float/2addr p1, p2

    iget p2, p0, Landroid/support/design/widget/BottomSheetBehavior;->b:I

    int-to-float p2, p2

    sub-float/2addr p1, p2

    invoke-static {p1}, Ljava/lang/Math;->abs(F)F

    move-result p1

    iget p2, p0, Landroid/support/design/widget/BottomSheetBehavior;->l:I

    int-to-float p2, p2

    div-float/2addr p1, p2

    const/high16 p2, 0x3f000000    # 0.5f

    cmpl-float p1, p1, p2

    if-lez p1, :cond_2e

    return v1

    :cond_2e
    return v3
.end method

.method public final b(Landroid/support/design/widget/CoordinatorLayout;Landroid/view/View;)Landroid/os/Parcelable;
    .registers 4
    .annotation system Ldalvik/annotation/Signature;
        value = {
            "(",
            "Landroid/support/design/widget/CoordinatorLayout;",
            "TV;)",
            "Landroid/os/Parcelable;"
        }
    .end annotation

    new-instance v0, Landroid/support/design/widget/BottomSheetBehavior$a;

    invoke-super {p0, p1, p2}, Landroid/support/design/widget/CoordinatorLayout$a;->b(Landroid/support/design/widget/CoordinatorLayout;Landroid/view/View;)Landroid/os/Parcelable;

    move-result-object p1

    iget p2, p0, Landroid/support/design/widget/BottomSheetBehavior;->d:I

    invoke-direct {v0, p1, p2}, Landroid/support/design/widget/BottomSheetBehavior$a;-><init>(Landroid/os/Parcelable;I)V

    return-object v0
.end method

.method final b(I)V
    .registers 3

    iget v0, p0, Landroid/support/design/widget/BottomSheetBehavior;->d:I

    if-ne v0, p1, :cond_5

    return-void

    :cond_5
    iput p1, p0, Landroid/support/design/widget/BottomSheetBehavior;->d:I

    iget-object p1, p0, Landroid/support/design/widget/BottomSheetBehavior;->g:Ljava/lang/ref/WeakReference;

    invoke-virtual {p1}, Ljava/lang/ref/WeakReference;->get()Ljava/lang/Object;

    return-void
.end method

.method public final b(Landroid/support/design/widget/CoordinatorLayout;Landroid/view/View;Landroid/view/MotionEvent;)Z
    .registers 7
    .annotation system Ldalvik/annotation/Signature;
        value = {
            "(",
            "Landroid/support/design/widget/CoordinatorLayout;",
            "TV;",
            "Landroid/view/MotionEvent;",
            ")Z"
        }
    .end annotation

    invoke-virtual {p2}, Landroid/view/View;->isShown()Z

    move-result p1

    const/4 v0, 0x0

    if-nez p1, :cond_8

    return v0

    :cond_8
    invoke-virtual {p3}, Landroid/view/MotionEvent;->getActionMasked()I

    move-result p1

    iget v1, p0, Landroid/support/design/widget/BottomSheetBehavior;->d:I

    const/4 v2, 0x1

    if-ne v1, v2, :cond_14

    if-nez p1, :cond_14

    return v2

    :cond_14
    iget-object v1, p0, Landroid/support/design/widget/BottomSheetBehavior;->e:Landroid/support/v4/widget/l;

    if-eqz v1, :cond_1b

    invoke-virtual {v1, p3}, Landroid/support/v4/widget/l;->b(Landroid/view/MotionEvent;)V

    :cond_1b
    if-nez p1, :cond_20

    invoke-direct {p0}, Landroid/support/design/widget/BottomSheetBehavior;->b()V

    :cond_20
    iget-object v1, p0, Landroid/support/design/widget/BottomSheetBehavior;->s:Landroid/view/VelocityTracker;

    if-nez v1, :cond_2a

    invoke-static {}, Landroid/view/VelocityTracker;->obtain()Landroid/view/VelocityTracker;

    move-result-object v1

    iput-object v1, p0, Landroid/support/design/widget/BottomSheetBehavior;->s:Landroid/view/VelocityTracker;

    :cond_2a
    iget-object v1, p0, Landroid/support/design/widget/BottomSheetBehavior;->s:Landroid/view/VelocityTracker;

    invoke-virtual {v1, p3}, Landroid/view/VelocityTracker;->addMovement(Landroid/view/MotionEvent;)V

    const/4 v1, 0x2

    if-ne p1, v1, :cond_58

    iget-boolean p1, p0, Landroid/support/design/widget/BottomSheetBehavior;->p:Z

    if-nez p1, :cond_58

    iget p1, p0, Landroid/support/design/widget/BottomSheetBehavior;->t:I

    int-to-float p1, p1

    invoke-virtual {p3}, Landroid/view/MotionEvent;->getY()F

    move-result v1

    sub-float/2addr p1, v1

    invoke-static {p1}, Ljava/lang/Math;->abs(F)F

    move-result p1

    iget-object v1, p0, Landroid/support/design/widget/BottomSheetBehavior;->e:Landroid/support/v4/widget/l;

    iget v1, v1, Landroid/support/v4/widget/l;->b:I

    int-to-float v1, v1

    cmpl-float p1, p1, v1

    if-lez p1, :cond_58

    iget-object p1, p0, Landroid/support/design/widget/BottomSheetBehavior;->e:Landroid/support/v4/widget/l;

    invoke-virtual {p3}, Landroid/view/MotionEvent;->getActionIndex()I

    move-result v1

    invoke-virtual {p3, v1}, Landroid/view/MotionEvent;->getPointerId(I)I

    move-result p3

    invoke-virtual {p1, p2, p3}, Landroid/support/v4/widget/l;->a(Landroid/view/View;I)V

    :cond_58
    iget-boolean p1, p0, Landroid/support/design/widget/BottomSheetBehavior;->p:Z

    if-nez p1, :cond_5d

    return v2

    :cond_5d
    return v0
.end method
